...
# دستورالعمل تحلیل
VISION_PROMPT = """
تصویر نمودار را با تمرکز روی محدوده‌ها و ترکیب سطوح فنی تحلیل کن:
...
۲. محاسبه دقیق فیبوناچی:
- فیبوناچی اصلاحی: از [سقف دقیق] به [کف دقیق] یا برعکس
  * 23.6% = [قیمت دقیق]
  * 38.2% = [قیمت دقیق]  
  * 50% = [قیمت دقیق]
  * 61.8% = [قیمت دقیق]
  * 78.6% = [قیمت دقیق]
- فیبوناچی اکستنشن: از [نقطه شروع] 
  * 127.2% = [قیمت دقیق]
  * 161.8% = [قیمت دقیق]
  * 200% = [قیمت دقیق]
  * 261.8% = [قیمت دقیق]

۳. تشخیص محدوده‌های کلیدی:
- محدوده‌های حمایتی: [قیمت پایین - قیمت بالا] بر اساس swing low ها و reaction های قبلی
- محدوده‌های مقاومتی: [قیمت پایین - قیمت بالا] بر اساس swing high ها و reaction های قبلی
- قدرت هر محدوده: ضعیف/متوسط/قوی (بر اساس تعداد reaction و حجم)

۴. شناسایی Confluence Zone ها:
- محدوده‌هایی که حمایت/مقاومت + فیبوناچی همپوشانی دارند
- مثال: "محدوده 1.150-1.180 = حمایت قوی + فیبو 61.8% (1.165)"
- اولویت‌بندی بر اساس تعداد همپوشانی‌ها

۵. تحلیل چند تایم‌فریمی:
- محدوده‌های مشترک بین تایم‌فریم‌ها
- تأیید confluence zone ها در تایم‌فریم‌های مختلف

۶. سناریوهای احتمالی:
- صعودی: شرط شکست [محدوده مقاومت]، اهداف [فیبو اکستنشن]
- نزولی: شرط شکست [محدوده حمایت]، اهداف [فیبو اکستنشن] 
- رنج: بین [محدوده] و [محدوده]

۷. سیگنال‌های منطقی (فقط در صورت R/R بالای ۱:۳):
**سیگنال شکست مقاومت:**
- ورود: بعد از شکست و تثبیت بالای [محدوده مقاومت]
- استاپ: زیر [محدوده حمایت نزدیک که شکستش تغییر روند دهد]
- تارگت ۱: [فیبو اکستنشن ۱۲۷.۲% یا ۱۶۱.۸%]
- تارگت ۲: [فیبو اکستنشن ۲۰۰% یا ۲۶۱.۸%]

**سیگنال برگشت از حمایت:**
- ورود: در [محدوده حمایت قوی/confluence zone]
- استاپ: زیر [محدوده حمایت که شکستش روند را تغییر دهد]
- تارگت ۱: [محدوده مقاومت نزدیک]
- تارگت ۲: [فیبو اکستنشن]

- R/R محاسبه دقیق: (مسافت تا تارگت) / (مسافت تا استاپ)
- منطق: [دلیل confluence zone یا شکست]

۸. مدیریت ریسک:
- حداکثر ریسک: ۱-۲٪ سرمایه
- استاپ منطقی: محدوده‌ای که شکستش تغییر کامل روند را نشان دهد
- محدوده هشدار: [قیمت] برای کاهش ۳۰-۵۰% پوزیشن
- خروج کامل: [همان استاپ ذکرشده]

۹. جمع‌بندی:
- مهم‌ترین confluence zone ها
- بهترین فرصت معاملاتی (حمایت قوی یا شکست مقاومت)
- توصیه نهایی با R/R دقیق

## اصول مهم:
- همه محدوده‌ها به شکل [X.XXX - X.XXX] ذکر شوند
- فیبوناچی‌ها حتماً محاسبه و در محدوده‌ها استفاده شوند
- سیگنال فقط با R/R حداقل ۱:۳ 
- استاپ جایی باشد که شکستش تغییر کامل روند دهد
- Confluence zone ها بر همپوشانی واقعی سطوح فنی باشند
"""
