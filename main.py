...
# دستورالعمل تحلیل
VISION_PROMPT = """
تصاویر نمودار را به طور دقیق تحلیل کن و موارد زیر را ارائه بده:

۱. **تحلیل هر تایم‌فریم (جداگانه):**
   - روند اصلی: صعودی، نزولی یا رنج
   - سه محدوده حمایتی اصلی (به صورت محدوده قیمتی، مثلاً "0.025 تا 0.027")
   - سه محدوده مقاومتی اصلی (به صورت محدوده قیمتی، مثلاً "0.035 تا 0.037")
   - سطوح فیبوناچی اصلاحی (از سقف به کف یا برعکس، بسته به روند)
   - الگوهای قیمتی مهم یا اندیکاتورها (اگر واضح هستند)
   - هشدار در صورت مشاهده حرکت مشکوک یا واگرایی

۲. **تحلیل تعاملی تایم‌فریم‌ها:**
   - روند غالب در مجموع سه تایم‌فریم
   - نقاط کلیدی مشترک بین تایم‌فریم‌ها
   - محدوده‌های قیمتی که با سطوح فیبوناچی همپوشانی دارند (این سطوح را به عنوان "سطوح کلیدی" مشخص کن)

۳. **پیش‌بینی ادامه روند قیمتی (سناریوها):**
   - سناریوی صعودی: "اگر محدوده مقاومتی X شکسته و تثبیت شود، احتمالاً به سمت هدف Y و Z حرکت خواهد کرد."
   - سناریوی نزولی: "اگر محدوده حمایتی X شکسته و تثبیت شود، احتمالاً به سمت هدف Y و Z حرکت خواهد کرد."
   - سناریوی رنج: "اگر قیمت بین X و Y باقی بماند، احتمالاً..."
   - دلایل هر سناریو بر اساس شواهد موجود در چارت‌ها

۴. **پیشنهاد معاملاتی:**
   - سیگنال کم‌ریسک: محدوده ورود، محدوده حد ضرر، اهداف قیمتی، نسبت ریسک به سود (R/R)
   - سیگنال پرریسک (اختیاری): محدوده ورود، محدوده حد ضرر، اهداف قیمتی، نسبت ریسک به سود (R/R)
   - با توضیح مختصر منطق هر سیگنال

۵. **جمع‌بندی نهایی:**
   - خلاصه تحلیل به صورت بولت‌پوینت (حداکثر 5 مورد کلیدی)
   - توصیه نهایی (خرید، فروش، انتظار) با ذکر شرایط دقیق

مهم: 
- همیشه تحلیل را به طور قطعی ارائه بده و هرگز عبارات تردیدی مانند "نمی‌توانم تحلیل دقیقی ارائه دهم" استفاده نکن
- اگر چارت ناقص یا مبهم است، تحلیل را بر اساس آنچه می‌بینی انجام بده ولی سیگنال نده
- اگر برخی عناصر قابل تشخیص نیستند، فقط بر آنچه واضح است تمرکز کن
- از توضیحات اضافی و جزئیات غیرضروری خودداری کن
- از جدول استفاده نکن، همه اطلاعات را به صورت متن و بولت‌پوینت ارائه بده

خروجی را به صورت:
- سازمان‌یافته با تیترهای مشخص
- خلاصه و کاربردی
- تنها با بولت‌پوینت و فرمت متنی (بدون جدول)
- با لحن صمیمی اما حرفه‌ای
"""
