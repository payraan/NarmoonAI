...
- سطوح فیبوناچی اصلاحی با اعداد دقیق (مثلاً: 38.2% = 1.250، 50% = 1.200، 61.8% = 1.150)
- سطوح فیبوناچی اکستنشن برای تارگت‌ها (مثلاً: 161.8% = 1.450، 200% = 1.550، 261.8% = 1.680)
...
۴. سناریوهای احتمالی:
- صعودی: شرط شکست مقاومت، اهداف قیمتی (با استفاده از فیبو اکستنشن) و احتمال
- نزولی: شرط شکست حمایت، اهداف قیمتی (با استفاده از فیبو اکستنشن) و احتمال
- رنج: محدوده حرکت و نقاط حساس شکست

۵. سیگنال عملی (در صورت وجود همه شرایط زیر):
- ورود (Entry): نقطه دقیق ورود
- محدوده هشدار (Warning Zone): سطح اول هشدار برای کاهش پوزیشن (۳۰-۵۰% خروج)
- خروج کامل (Full Exit): سطح قطعی خروج از معامله (۱۰۰% خروج)
- تارگت ۱: بر اساس نزدیک‌ترین مقاومت یا فیبو اکستنشن 161.8%
- تارگت ۲: بر اساس فیبو اکستنشن 200% یا 261.8%
- R/R: محاسبه بر اساس خروج کامل (حداقل ۱:۲.۵ یا بهتر)
- شکست یا پولبک تأیید شده با کندل قوی و حجم (۲ برابر میانگین)
- منطق کوتاه سیگنال (مثلاً "برگشت از فیبو ۶۱.۸٪ در ۱.۱۵۰ + تایید حجم")

۶. مدیریت ریسک داینامیک:
- حداکثر ریسک: ۱-۲٪ سرمایه (بر اساس خروج کامل)
- استراتژی خروج مرحله‌ای:
  * هشدار: کاهش ۳۰-۵۰% پوزیشن در محدوده هشدار
  * خروج کامل: بستن کل پوزیشن در سطح خروج کامل
- فاصله بین محدوده هشدار و خروج کامل: حداقل ۲-۳% برای تحمل نوسانات
- هشدار ویژه: احتمال فیک بریک‌اوت، نوسان بالا، اخبار مؤثر

۷. جمع‌بندی و توصیه نهایی:
- خلاصه روند و نکته کلیدی معامله (تا ۵ بولت‌پوینت)
- توصیه نهایی: خرید/فروش/انتظار (با شرط دقیق)
- تأکید بر استراتژی خروج مرحله‌ای

مهم:
- محدوده هشدار و خروج کامل باید در تمام بخش‌ها یکسان باشند
- فیبوناچی‌ها با اعداد دقیق قیمتی ذکر شوند
- از فیبوناچی اکستنشن برای تارگت‌ها استفاده شود
- R/R بر اساس خروج کامل محاسبه شود (نه محدوده هشدار)
- فاصله کافی بین دو سطح خروج برای مدیریت بهتر ریسک
- خروجی حرفه‌ای، عملیاتی و بدون تناقض باشد
"""
