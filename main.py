...
# دستورالعمل تحلیل
VISION_PROMPT = """
تصویر نمودار را فقط بر اساس داده تصویری و طبق ساختار زیر به صورت بولت‌پوینت و بدون جدول تحلیل کن:

۱. پردازش اولیه:
- تایم فریم و جفت ارز را شناسیایی کن
- روند هر تایم‌فریم: صعودی/نزولی/رنج
- قیمت فعلی و نوع آخرین کندل (مثلاً مارابوزو، دوجی، پین‌بار)

۲. تحلیل سطوح و الگوها:
- سه حمایت و سه مقاومت کلیدی (Swing High/Low، اعداد رند، سطوح تاریخی)
- قدرت هر سطح: ضعیف/متوسط/قوی (بر اساس برخورد و حجم)
- سطوح فیبوناچی اصلی (38.2%، 50%، 61.8%، 161.8%)
- مهم‌ترین الگو یا کندل ویژه (مثلاً انگالفینگ یا پین‌بار روی سطح)
...
۴. سناریوهای احتمالی:
- صعودی: شرط شکست مقاومت، اهداف قیمتی و احتمال
- نزولی: شرط شکست حمایت، اهداف قیمتی و احتمال
- رنج: محدوده حرکت و نقاط حساس شکست

۵. سیگنال عملی (در صورت وجود همه شرایط زیر):
- ورود (Entry)، استاپ، تارگت ۱ و ۲، R/R (حداقل ۱:۲)
- شکست یا پولبک تأیید شده با کندل قوی و حجم (۲ برابر میانگین)
- منطق کوتاه سیگنال (مثلاً "برگشت از فیبو ۶۱.۸٪ + تایید حجم")

۶. مدیریت ریسک و هشدار:
- حداکثر ریسک: ۱-۲٪ سرمایه
- استاپ‌لاس منطقی و حرفه‌ای (بر اساس ساختار چارت)
- هشدار یا ریسک ویژه (مثل فیک بریک‌اوت، نوسان بالا یا خبر مهم)

۷. جمع‌بندی و توصیه نهایی:
- خلاصه روند و نکته کلیدی معامله (تا ۵ بولت‌پوینت)
- توصیه نهایی: خرید/فروش/انتظار (با شرط دقیق)
